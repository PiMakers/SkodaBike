...
//                ofLogNotice() << "  Speed (m/s): " <<  speed_mps << "  Speed (km/h): " << speed_kmph;
        ...
        else ofResetElapsedTimeCounter();
...
///    gui.setup(parameters,"mysettings.xml" ,50, 200);
    gui.setup(dmxSettings,"DMXsettings.xml" ,300, 200);

    gui1.setName("Parameters");
    gui1.setPosition( 50,300);
    gui1.setShape(60, 100, 150, 560);
    gui1.setup(parameters,"Psettings.xml" ,10, 10);
...
//    lampLevel = (int)255*k;
    ...
    else if (speed_kmph < 25) lampLevel = 50;
    else if (speed_kmph < 40) lampLevel = 255;
    else lampLevel = 300;

//    ofLogNotice() << "  ElapsedTime: " << ofGetElapsedTimeMillis();    
    if (ofGetElapsedTimeMillis() > 2000 ) lampLevel = 0;

        ofxOscMessage m;
/*        if (dmx1) m.setAddress("/dmx1/level");
        else if (dmx2) 
*/      m.setAddress("/dmx2/level");
        m.addIntArg(lampLevel);
